...
class IngestPostUrlRequest(BaseModel):
    url: str
    reciter_slug: str = config.DEFAULT_RECITER_SLUG

@app.post("/api/v1/ingest/url")
def ingest_post_by_url(body: IngestPostUrlRequest):
    """
    Downloads audio and ingests metadata directly from any Telegram post URL or media link.
    """
    crawler = TelegramChannelCrawler(reciter_slug=body.reciter_slug, db=db)
    track_id = crawler.ingest_post_url(body.url)
    if not track_id:
        raise HTTPException(status_code=400, detail=f"Failed to ingest media from URL: {body.url}")
    return {
        "status": "success",
        "track_id": track_id,
        "url": body.url
